...
Modify the removeManyPeople function to delete all the people whose name 
is within the variable nameToRemove, using Model.remove(). 
...
const removeManyPeople = (done) => {
  ...
  Person.remove({name: nameToRemove}, (err, jsonObj) => {
      done(null, jsonObj);
  });
};
...
the query is not executed. You can store the query in a variable for later use. 
...
const queryChain = (done) => {
  ...
  Person.find({favoriteFoods: foodToSearch}).sort({name: 1}).limit(2).select({age: 0}).exec((err,match) => {
    if(err) return console.log(err);
    done(null,match);
  });
};
